...
//--METODOS
	...
	public static function Guardar($obj)
	{
		try {
			...
			$vehiculoExistente = Vehiculo::TraerVehiculoPorPatente($vehiculo->getPatente());
			//Agrego el vehiculo en caso que sea su primer ingreso
	...
	public static function traerCaja($fechaDesde, $fechaHasta){
		try {
			$resultado = array();
			$objConexion = Conexion::getConexion();

			$query = "SELECT SUM(costo) as costo, 
							COUNT(*) as vehiculos
					FROM operaciones
					WHERE egreso IS NOT NULL";

			if($fechaDesde != NULL && $fechaHasta != NULL){
				$query .= " AND egreso between '".$fechaDesde."' AND '".$fechaHasta."'";
			};
